...
class CRM_Dedupe_BAO_QueryBuilder {
  ...
  public static function internalFilters($rg, $strID1 = 'contact1.id', $strID2 = 'contact2.id') {
    ...
    if (!empty($rg->contactIds)) {
      ...
      // DedupeGroup Custom Change START - Make sure both cid1 and cid2 is in the group contacts.
      $group_dupe = CRM_Utils_Request::retrieve('group_dupe', 'Positive');
      if (!empty($group_dupe)) {
        return "($strID1 IN ($cids) AND (($strID2 IN ($cids) AND $strID1 < $strID2) ))";
      }
      // DedupeGroup Custom Change END
